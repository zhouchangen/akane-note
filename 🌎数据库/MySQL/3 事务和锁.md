...
**不可重复读：两次读的数据不一样**，一个事务在读取某些数据已经发生update！这种现象叫做“不可重复读”。 

比如：在读可提交（RC）的隔离级别下，A事务开始，读取了一次数据，这时候B事务修改了一条数据，这里A事务又读取了一次数据，就会发现两次读的数据不一样了。



**幻读： 幻读指的是一个事务在前后两次查询同一个范围的时候，**后一次查询看到了前一次查询没有看到的行**。

比如：读的时候别人后面又插入了一条数据 (MySQL增加了间隙锁解决幻读问题)
