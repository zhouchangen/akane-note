...
Hash是利用了数组快速查询的特点，但缺点是不能范围查询，并且数据重复会产生链表。
...
红黑树比起二叉树，改进的一点就是会自动平衡，但是由于数据量上来时，还是会导致树太高。
...
B+ Trees是对B Trees的改进，可以看到，每个叶子节点都指向相邻的叶子节点的地址
...
### 聚簇索引和非主键索引， 唯一索引和普通索引

- 聚簇索引的叶子节点**存的是整行数据**。在 InnoDB 里，主键索引也被称为聚簇索引（clustered index）**。**
- 非主键索引的叶子节点**内容是主键的值**。在 InnoDB 里，非主键索引也被称为二级索引（secondary index）。
...
如果查询的值已经在索引上了，不需要再回表查询，则称为覆盖索引。
